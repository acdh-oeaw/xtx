...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xtoks="http://acdh.oeaw.ac.at/xtoks" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs tei" version="2.0">
    ...
    <xsl:key name="lex-entry-by-part" match="tei:seg" use="(tei:w|tei:pc|tei:seg[@type = 'ws'])/normalize-space(.)"/>
    ...
    <xsl:template match="tei:w" mode="tag-parts" priority="1">
        <xsl:variable name="part" as="xs:string?">
            <xsl:choose>
                ...
                <xsl:when test="preceding-sibling::*[1]/self::tei:w and not(following-sibling::*[1]/self::tei:w)">F</xsl:when><!--<xsl:when test="not(preceding-sibling::*[1]/self::tei:w and following-sibling::*[1]/self::tei:w)">NO</xsl:when>-->
                <xsl:otherwise/>
            </xsl:choose>
        </xsl:variable>
        <xsl:copy>
    ...
    <xsl:template match="tei:*[key('lex-entry-by-part',normalize-space(.),$lexToks)]" mode="applyLex" priority="1">
        ...
        <xsl:variable name="lex-entries" as="item()*">
            <xsl:for-each select="$segCandidates"><!-- number of tokens in the lexicon entry -->
                <xsl:variable name="totalToks" select="count(*)"/><!-- position of the current token in the lexicon-token -->
                ...
                <xsl:if test="$debug">
                    ...
                    <xsl:message>total toks / before / after : <xsl:value-of select="$totalToks"/> / <xsl:value-of select="$toks-before"/> / <xsl:value-of select="$toks-after"/>
                    </xsl:message>
                    <xsl:message>prev / next : <xsl:value-of select="string-join($prev,'')"/> / <xsl:value-of select="string-join($next,'')"/>
                    </xsl:message>
                    <xsl:message>$parts-joined: <xsl:value-of select="$parts-joined"/>
                    </xsl:message>
                </xsl:if>
                <xsl:if test="$parts-joined = $lex-entry">
                    <entry id="{@xml:id}">
                        ...
                        <toks-before>
                            <xsl:value-of select="$toks-before"/>
                        </toks-before>
                        <toks-after>
                            <xsl:value-of select="$toks-after"/>
                        </toks-after>
                    </entry>
                </xsl:if>
            </xsl:for-each>
        ...
        <xsl:choose>
            <xsl:when test="count($lex-entries) &gt; 0"><!--                <xsl:message>$lex-entry <xsl:value-of select="$lex-entries[1]/@id"/> "<xsl:value-of select="$lexToks//tei:seg[@xml:id = $lex-entries[1]/@id]"/>"</xsl:message>-->
        ...
        <xsl:copy>
            ...
            <xsl:if test="$in-toks-list">
                ...
                <xsl:attribute name="part">
                    <xsl:choose>
                        <xsl:when test="$in-toks-list/following-sibling::*">
                            <xsl:text>M</xsl:text>
                            <xsl:message>
                                <xsl:copy-of select="$in-toks-list/following-sibling::*"/>
                            </xsl:message>
                        </xsl:when>
    ...
    <xsl:template match="tei:*[@part = ('M','F')]" mode="collapseParts-tag">
        <xsl:param name="partsTagged" as="document-node()" tunnel="yes"/>
        <xtoks:token corresp="#{@xml:id}">
            <xsl:copy-of select="(@part|@prev|@next)"/>
        </xtoks:token>
        <xsl:if test="@next">
    ...
    </xsl:template><!--    <xsl:template match="xtoks:collapsed" mode="expandCollapsed" priority="1"/>-->
    ...
    <xsl:template match="*[@part != 'F' and not(@next)]" mode="addNext" priority="4">
        <xsl:copy>
            <xsl:copy-of select="@*"/>
            <xsl:attribute name="next">#<xsl:value-of select="following::*[substring-after(@prev,'#') = current()/@xml:id]/@xml:id"/>
            </xsl:attribute>
            <xsl:if test="@part != 'I' and not(@prev)">
                <xsl:attribute name="prev">#<xsl:value-of select="preceding::*[substring-after(@next,'#') = current()/@xml:id]/@xml:id"/>
                </xsl:attribute>
            </xsl:if>
            <xsl:copy-of select="node()"/>
        </xsl:copy>
